...
class RadarChart extends StatefulWidget {
  ...
  const RadarChart({
    ...
    @required this.data,
    this.showTicksText = false,
    this.reverseAxis = false,
    this.ticksTextStyle = const TextStyle(color: Colors.grey, fontSize: 12),
    this.featuresTextStyle = const TextStyle(color: Colors.black, fontSize: 14),
...
class RadarChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    List<Paint> ticksPainter = [
      Paint()
        ..color = graphBgColor[0]
        ..style = PaintingStyle.fill
        ..strokeWidth = 1.0
        ..isAntiAlias = true,
      Paint()
        ..color = graphBgColor[1]
        ..style = PaintingStyle.fill
        ..strokeWidth = 1.0
        ..isAntiAlias = true,
      Paint()
        ..color = graphBgColor[2]
        ..style = PaintingStyle.fill
        ..strokeWidth = 1.0
        ..isAntiAlias = true,
    ];
    ...
    data.asMap().forEach((index, graph) {
      var graphPaint = Paint()
        ..color = Styles.radarColor.withOpacity(0.3)
        ..style = PaintingStyle.fill;

      var graphOutlinePaint = Paint()
        ..color = Styles.radarColor
        ..style = PaintingStyle.stroke
        ..strokeWidth = 2.0
        ..isAntiAlias = true;
      ...
      graph.asMap().forEach((index, point) {
        ...
        if (reverseAxis) {
          ...
          canvas.drawCircle(
              Offset(centerX + (radius * fraction - scaledPoint) * xAngle,
                  centerY + (radius * fraction - scaledPoint) * yAngle),
              20,
              circlePaint);
        } else {
          ...
          canvas.drawCircle(
              Offset(centerX + scaledPoint * xAngle,
